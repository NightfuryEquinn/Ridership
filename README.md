...
|------|-------|-----------|----|-----|------|----------------|
| 1 | **HMT-TSF-FR** (F=53) | **81.62%** | **0.807** | 60,306 | **101,291** | Yes — both |
| 2 | HMT-TSF (F=79) | 81.46% | 0.797 | **59,105** | 103,684 | Yes — both |
...
**HMT-TSF-FR** sets the study-wide headline at nomco·lb14: Combined% 81.62%, R² 0.807, lowest RMSE (101,291). The next best baseline (Informer tuned) is 1.63 pp below. Full baseline and tuned results are in `src/models/MODEL.md`; HMT-TSF cross-configuration results are in `src/models/hybrid/HMT-TSF.md`.
...
Graph-based models suffer the most under MCO: their static Pearson-correlation adjacency is fitted on normal-period training data and cannot adapt to the COVID-driven ridership collapse. Informer and ST-LSTM are most resilient (< −5 pp).
...
| Model | Key Architecture | Strengths | Weaknesses |
|-------|-----------------|-----------|------------|
| **HMT-TSF** | RevIN → 5-group semantic feature encoders → Temporal Transformer block → [Multi-Scale TCN ‖ Feature GCN ‖ Regime Gating] → SE-gated fusion → dual forecast heads; optional CatBoost residual correction | Best study-wide: Combined% 81.62%, R² 0.807 (nomco_lb14, FR variant); regime gating handles MCO structural break; widest lookback support (7–84 days); SHAP-guided feature reduction (79→53 features) further improves accuracy | Most complex architecture; many interacting hyperparameters; SHAP and CatBoost add significant runtime; mco_lb84 collapses when static spatial features are removed (−4.42 pp) |
