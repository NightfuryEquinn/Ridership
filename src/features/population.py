...
# ══════════════════════════════════════════════════════════════════════════════
# NEW: State-level aggregation
# ══════════════════════════════════════════════════════════════════════════════
# Assigns each grid point to a state polygon (spatial join), then computes
# mean and total density per state.
# Output: data/cleaned/population_by_state.csv  — 16 rows × state metrics
...
if SPATIAL_AVAILABLE:
    ...
    try:
        ...
        # Convert population grid to GeoDataFrame
        ...
        # Spatial join: assign each grid point to a state
...
# ══════════════════════════════════════════════════════════════════════════════
# NEW: Stop-level sampling
# ══════════════════════════════════════════════════════════════════════════════
# For each GTFS stop, find the nearest grid cell and extract its density.
# This produces a static node feature (density at stop location) used by
# GCN-based models as part of the node feature matrix.
# Output: data/cleaned/population_at_stops.csv
...
if SPATIAL_AVAILABLE:
    ...
    if stop_frames:
        ...
        try:
            ...
            # Build BallTree on grid points for fast nearest-neighbor lookup
            ...
            df_stops["nearest_grid_dist_km"] = dist.flatten() * 6371  # haversine → km
