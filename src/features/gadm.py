...
# Each state node needs a (lat, lon) for spatial embedding / distance-based
# edge weights in TGACN / STGNN-STEP.
node_records = []
...
# Two states are adjacent if their geometries share a border (intersection
# length > 0). Self-loops are excluded (diagonal = 0) unless your GCN
# formulation adds them explicitly (Ã = A + I).
#
# Edge weights = shared border length in degrees (convert to km: × 111 km/deg)
# for use in distance-weighted adjacency in TGACN / STGNN-STEP.
