...
# ── 3–4. Deduplicate and build cleaned list ───────────────────────────────────
seen_ids = set()
cleaned  = []
...
# ══════════════════════════════════════════════════════════════════════════════
# NEW: POI catchment aggregation around GTFS stops
# ══════════════════════════════════════════════════════════════════════════════
# For each transit stop, count POIs within CATCHMENT_RADIUS_M metres.
# POI categories are derived from OSM amenity / shop / leisure tags.
# The resulting per-stop counts become static GCN node features.

CATCHMENT_RADIUS_M = 500    # metres — tunable; 300–800 m is typical for transit

# OSM tag → category mapping
# Extend as needed; anything not matched falls into "other"
CATEGORY_MAP = {
    # Transport
...
def aggregate_poi_counts(
    ...
    # Build array of POI coordinates
    ...
    # Convert radius from metres to radians
...
# Load GTFS stop nodes (all operators merged)
stop_frames = []
...
if stop_frames:
    ...
    if not df_poi_counts.empty:
        # Log-transform counts to reduce skew (dense urban stops vs rural)
