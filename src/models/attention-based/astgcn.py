...
Architecture (one ASTGCN block):
  X (B, T, N, d)
  ──► Spatial Attention   (over N nodes at each T)  → multi-head SA
...
def parse_args():
    p = argparse.ArgumentParser(description="ASTGCN forecaster — 12-way comparison")
    ...
    p.add_argument("--n-blocks",             type=int,   default=2,
                   help="Number of stacked ASTGCN blocks")
    p.add_argument("--K",                    type=int,   default=3,
                   help="Chebyshev polynomial order")
    p.add_argument("--n-heads",              type=int,   default=4,
                   help="Attention heads (spatial and temporal)")
...
def build_adj_and_laplacian(X_train: torch.Tensor, threshold: float = 0.1):
    ...
    L_tilde = 2 * L_norm / λ_max − I
    L_norm  = I − D^{-½} A D^{-½}
    λ_max   ≈ 2 (upper bound for normalised Laplacian)
    → L_tilde = −D^{-½} A D^{-½}  (the normalised adjacency with neg sign)

    Returns both as float32 tensors of shape (N, N).
    """
    ...
    # Symmetrically normalised adjacency  (no self-loops for L; self-loops for A_norm)
    ...
    # Normalised Laplacian: L = I − D^{-½} A D^{-½}  (without self-loops)
    D        = A.sum(axis=1)
    D_inv_sq2 = 1.0 / np.sqrt(D + 1e-9)
    A_sym    = D_inv_sq2[:, None] * A * D_inv_sq2[None, :]
    L_norm   = np.eye(A.shape[0], dtype=np.float32) - A_sym

    # Scaled Laplacian: L_tilde = 2L/2 − I = L − I = −A_sym
    L_tilde  = L_norm - np.eye(L_norm.shape[0], dtype=np.float32)  # = -A_sym
...
class ChebConv(nn.Module):
    ...
    Computes T_0(L̃)X, T_1(L̃)X, ..., T_{K-1}(L̃)X and applies a learned
    linear combination, where T_k is the k-th Chebyshev polynomial and
    L̃ = 2L/λ_max − I is the scaled graph Laplacian.
...
class ASTGCNBlock(nn.Module):
    ...
    def forward(self, X: torch.Tensor, L: torch.Tensor) -> torch.Tensor:
        ...
        # 1. Spatial attention at each timestep
        ...
        # 3. Temporal attention for each node
        ...
        X  = Xt.reshape(B, N, T, D).permute(0, 2, 1, 3)  # (B, T, N, D)
        return X
...
class ASTGCNForecaster(nn.Module):
    ...
    def __init__(
        ...
        # Project scalar node signal → d_model
        ...
        # Output: mean-pool over N → flatten T_in × d → MLP → T_out
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        t_pos = self.t_embed(t_idx)                       # (T, d_model)
        X = X + t_pos.unsqueeze(0).unsqueeze(2)           # (B, T, N, d_model)

        # ASTGCN blocks
        for block in self.blocks:
            X = block(X, self.L)                          # (B, T, N, d_model)

        # Mean pool over nodes → flatten → MLP
        X   = X.mean(dim=2)                               # (B, T, d_model)
        X   = X.reshape(B, -1)                            # (B, T*d_model)
        return self.head(X)                               # (B, T_out)


# ══════════════════════════════════════════════════════════════════════════════
# Metrics  (identical across all models)
# ══════════════════════════════════════════════════════════════════════════════
...
def plot_predictions(y_true, y_pred, metrics, out_path):
    ...
    ax1.plot(idx_full, predicted_s1, color=C_PRED, linewidth=1.2, linestyle="--", alpha=0.88,
             label="ASTGCN predicted (step 1)", zorder=5)
    ...
    ax1.set_title("ASTGCN — Test set: Actual vs Predicted (full period)", fontsize=11, fontweight="bold")
...
def plot_per_step_metrics(per_step, out_path):
    ...
    ax1.set_title("ASTGCN — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²"); ax2.set_title("ASTGCN — R² per horizon", fontweight="bold")
    ax2.grid(alpha=0.3)
    fig.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(fig); print(f"  Saved: {out_path}")
...
def main():
    ...
    W = 10; print(f"\n{'='*85}"); print("ASTGCN — TEST SET METRICS"); print(f"{'='*85}")
    ...
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S"); out_dir = f"src/outputs/astgcn/{run_id}"
    ...
    print(f"\n{'='*85}"); print(f"ASTGCN run complete  →  {out_dir}/"); print(f"{'='*85}")
    ...
    if len(models_data) > 1:
        print_comparison_table(models_data[:-1], overall, "ASTGCN")
    ...
    print(f"  ASTGCN vs naive     ΔCombined={d_comb:+.2f}%  ΔMAPE={d_mape:+.2f}%")
