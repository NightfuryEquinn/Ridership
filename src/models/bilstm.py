...
def compare_with_baseline(bilstm_metrics: dict, baseline_path: str):
    ...
    # Lower-is-better for all except R2 and Combined
    ...
    logging.info("=" * len(sep))
    logging.info(f"  {'Metric':<12}{'BiLSTM v4':>{col_w}}{'LSTM v4':>{col_w}}{'Winner':>{col_w}}")
    logging.info(sep)
    ...
    for m in metrics_order:
        ...
        if m in higher_better:
            winner = "BiLSTM (W)" if bi_val > base_val else ("LSTM (W)" if base_val > bi_val else "TIE")
        else:
            winner = "BiLSTM (W)" if bi_val < base_val else ("LSTM (W)" if base_val < bi_val else "TIE")

        fmt = ".4f" if m in {"NMAE", "NRMSE", "R2"} else ".2f"
        logging.info(
            f"  {m:<12}{bi_val:>{col_w}{fmt}}{base_val:>{col_w}{fmt}}{winner:>{col_w}}"
        )
