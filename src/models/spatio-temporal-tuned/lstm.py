...
def plot_loss_curves(train_losses: list, val_losses: list, out_path: str) -> None:
    ...
    ax.plot(train_losses, label="Train Loss", linewidth=1.5, color="#2563eb")
    ax.plot(val_losses,   label="Val Loss",   linewidth=1.5, color="#f97316", linestyle="--")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss (scaled)")
    ax.set_title("LSTM Tuned — Training curves")
    ax.legend()
    ax.grid(alpha=0.3)
    fig.tight_layout()
...
def plot_predictions(
    ...
    ax1.set_title(
        "LSTM Tuned — Test set: Actual vs Predicted (full period)",
...
def main():
    ...
    print(f"\nModel : LSTMForecaster (tuned)  hidden={args.hidden}  layers={args.layers}")
    ...
    out_dir = f"src/outputs/lstm_tuned/{run_id}"
