...
def parse_args():
    p = argparse.ArgumentParser(
        description="Tuned CNN-LSTM forecaster"
    )
...
class CNNLSTMForecaster(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "sequential":
            ...
            h_T = h_n[-1]
            skip = x.mean(dim=1)
            h_cat = torch.cat([h_T, skip], dim=-1)
            return self.head(h_cat)

        else:  # parallel
            cnn_out = x_cnn.mean(dim=-1)
            _, (h_n, _) = self.lstm(x)
            h_T = h_n[-1]
            h_cat = torch.cat([cnn_out, h_T], dim=-1)
            return self.head(h_cat)
...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.set_title("CNN-LSTM Tuned — Training curves")
...
def plot_predictions(
    ...
    ax1.set_title("CNN-LSTM Tuned — Test set: Actual vs Predicted (full period)",
...
def plot_per_step_metrics(per_step: list, out_path: str) -> None:
    ...
    ax1.set_title("CNN-LSTM Tuned — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²")
    ax2.set_title("CNN-LSTM Tuned — R² per horizon step", fontweight="bold")
    ax2.grid(alpha=0.3)
...
def main():
    ...
    if args.mode == "sequential":
        ...
        lstm_in = args.cnn_filters
    print(f"\nModel     : CNNLSTMForecaster (tuned) [{args.mode}]")
    ...
    # ── Per-horizon metrics ───────────────────────────────────────────────────
    W = 10
    mode_label = f"CNN-LSTM Tuned [{args.mode}]"
    print(f"\n{'='*85}")
    ...
    out_dir = f"src/outputs/cnn_lstm_tuned/{run_id}"
