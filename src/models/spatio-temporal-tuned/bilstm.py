...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.plot(train_losses, label="Train Loss", linewidth=1.5, color="#7c3aed")
    ax.plot(val_losses,   label="Val Loss",   linewidth=1.5, color="#f97316", linestyle="--")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss (scaled)")
    ax.set_title("BiLSTM Tuned — Training curves")
    ax.legend()
    ax.grid(alpha=0.3)
    fig.tight_layout()
...
def plot_predictions(
    ...
    ax1.set_title("BiLSTM Tuned — Test set: Actual vs Predicted (full period)",
...
def plot_per_step_metrics(per_step: list, out_path: str) -> None:
    ...
    ax1.set_title("BiLSTM Tuned — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²"); ax2.set_title("BiLSTM Tuned — R² per horizon step", fontweight="bold")
    ax2.grid(alpha=0.3)
...
def main():
    ...
    print(f"\nModel  : BiLSTMForecaster (tuned)  hidden={args.hidden}  layers={args.layers}")
    ...
    W = 10
    print(f"\n{'='*85}")
    print("BiLSTM Tuned — TEST SET METRICS")
    print(f"{'='*85}")
    header = (
        f"{'Step':>5}  "
    ...
    out_dir = f"src/outputs/bilstm_tuned/{run_id}"
    ...
    results = {
        ...
        "hparams": {
            ...
            "loss": args.loss, "warmup_epochs": args.warmup_epochs,
        },
        "training": {
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*85}")
    print(f"BiLSTM Tuned run complete  →  {out_dir}/")
    print(f"{'='*85}")
