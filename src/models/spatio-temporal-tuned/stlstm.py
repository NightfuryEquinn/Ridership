...
def parse_args():
    p = argparse.ArgumentParser(
        description="Tuned ST-LSTM forecaster"
    )
...
class STLSTMForecaster(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sp = self.spatial_enc(x)

        h_cat = torch.cat([h_T, sp], dim=-1)
...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.set_title("ST-LSTM Tuned — Training curves")
...
def plot_predictions(
    ...
    ax1.set_title("ST-LSTM Tuned — Test set: Actual vs Predicted (full period)",
...
def plot_per_step_metrics(per_step: list, out_path: str) -> None:
    ...
    ax1.set_title("ST-LSTM Tuned — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²")
    ax2.set_title("ST-LSTM Tuned — R² per horizon step", fontweight="bold")
    ax2.grid(alpha=0.3)
...
def main():
    ...
    W = 10
    print(f"\n{'='*85}")
    print("ST-LSTM Tuned — TEST SET METRICS")
    print(f"{'='*85}")
    header = (
        f"{'Step':>5}  "
    ...
    out_dir = f"src/outputs/st_lstm_tuned/{run_id}"
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*85}")
    print(f"ST-LSTM Tuned run complete  →  {out_dir}/")
    print(f"{'='*85}")
