...
# ─────────────────────────────────────────────
def compare_with_baseline(step_metrics: dict, baseline_path: str):
    """
    ...
    Run lstm_v4.py first, then re-run stgnn_step_v4.py.
    """
    ...
    logging.info("  MODEL COMPARISON — STGNN-STEP Enhanced v4 vs LSTM v4 (baseline)")
    logging.info("=" * len(sep))
    logging.info(f"  {'Metric':<12}{'STGNN-STEP v4':>{col_w}}{'LSTM v4':>{col_w}}{'Winner':>{col_w}}")
    logging.info(sep)

    for m in metrics_order:
        step_val = step_metrics.get(m, float("nan"))
        base_val = baseline.get(m, float("nan"))

        if m in higher_better:
            winner = "STGNN-STEP (WIN)" if step_val > base_val else ("LSTM (WIN)" if base_val > step_val else "TIE")
        else:
            winner = "STGNN-STEP (WIN)" if step_val < base_val else ("LSTM (WIN)" if base_val < step_val else "TIE")

        fmt = ".4f" if m in {"NMAE", "NRMSE", "R2"} else ".2f"
        logging.info(
            f"  {m:<12}{step_val:>{col_w}{fmt}}{base_val:>{col_w}{fmt}}{winner:>{col_w}}"
        )
