...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    ...
    return {"Combined": combined, "MAPE": mape, "MAE_pct": mae_pct,
            "RMSE_pct": rmse_pct, "R2": r2, "MAE": mae_raw, "RMSE": rmse_raw}
...
def load_model_results(path, model_dir, label):
    if path:
        if not os.path.exists(path):
            print(f"[WARN] {label} results not found at: {path}"); return None
        with open(path) as f: return json.load(f)
    candidates = sorted(glob.glob(f"{model_dir}/**/results.json", recursive=True))
    if not candidates:
        print(f"[INFO] No {label} results.json found under {model_dir} — skipping."); return None
...
def plot_comparison(models_data, out_path):
    ...
    n_models=len(models_data); w_bar=max(0.08,0.80/n_models)
    ...
    for (name,_,ps,col),mk,ls in zip(models_data,markers,styles):
        ax10.plot(steps,[m["Combined"] for m in ps[:n_steps]],marker=mk,color=col,linewidth=1.8,markersize=5,label=name,linestyle=ls)
    ax10.set_ylabel("Combined%");ax10.set_title("Combined% per Horizon",fontweight="bold");ax10.legend(fontsize=6);ax10.grid(alpha=0.3)
    ax11=fig.add_subplot(gs[1,1])
    for (name,_,ps,col),mk,ls in zip(models_data,markers,styles):
        ax11.plot(steps,[m["R2"] for m in ps[:n_steps]],marker=mk,color=col,linewidth=1.8,markersize=5,label=name,linestyle=ls)
    ax11.axhline(0,color="#9ca3af",linewidth=0.8,linestyle="--");ax11.axhline(1,color="#16a34a",linewidth=0.8,linestyle=":")
    ax11.set_ylabel("R²");ax11.set_title("R² per Horizon",fontweight="bold");ax11.legend(fontsize=6);ax11.grid(alpha=0.3)
    ax20=fig.add_subplot(gs[2,0])
    for (name,_,ps,col),mk,ls in zip(models_data,markers,styles):
        ax20.plot(steps,[m["MAE"] for m in ps[:n_steps]],marker=mk,color=col,linewidth=1.8,markersize=5,label=name,linestyle=ls)
    ax20.set_ylabel("MAE (riders)");ax20.set_title("Raw MAE per Horizon",fontweight="bold");ax20.legend(fontsize=6);ax20.grid(alpha=0.3)
    ax21=fig.add_subplot(gs[2,1])
    for (name,_,ps,col),mk,ls in zip(models_data,markers,styles):
        ax21.plot(steps,[m["RMSE"] for m in ps[:n_steps]],marker=mk,color=col,linewidth=1.8,markersize=5,label=name,linestyle=ls)
    ax21.set_ylabel("RMSE (riders)");ax21.set_title("Raw RMSE per Horizon",fontweight="bold");ax21.legend(fontsize=6);ax21.grid(alpha=0.3)
    fig.suptitle(" vs ".join(d[0] for d in models_data)+" — Test Set Comparison",
