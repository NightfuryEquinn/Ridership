...
def parse_args():
    p = argparse.ArgumentParser(
        description="TPA-LSTM forecaster with LSTM + BiLSTM comparison"
    )
    p.add_argument("--seq-dir",        default="data/sequences/lstm",
                   help="Directory with X/y .npy splits (shared with LSTM/BiLSTM)")
...
class TemporalPatternAttention(nn.Module):
    ...
    def __init__(self, hidden_size: int, n_filters: int, kernel_size: int):
        ...
        # Conv1d: in_channels=hidden_size (one per hidden dim over time)
        #         out_channels=n_filters, kernel_size operates over the time axis
        self.conv = nn.Conv1d(
            in_channels  = hidden_size,
            out_channels = n_filters,
            kernel_size  = kernel_size,
            padding      = kernel_size // 2,   # same-padding preserves length
        )

        # Scoring: maps each filter's pooled output to a scalar against h_T
        # W_score projects h_T from hidden_size → n_filters space
        self.W_score = nn.Linear(hidden_size, n_filters, bias=False)

        # Project context back to hidden_size for concatenation with h_T
        self.W_context = nn.Linear(n_filters, hidden_size, bias=False)

    def forward(
        self,
        H:   torch.Tensor,   # (B, T_in, hidden_size) — all LSTM hidden states
        h_T: torch.Tensor,   # (B, hidden_size)       — final hidden state
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Returns
        -------
        context     : (B, hidden_size)  — attention-weighted pattern context
        attn_weights: (B, n_filters)    — softmax weights (for visualisation)
        """
        B, T, H_dim = H.shape

        # 1. Context hidden states (exclude last — h_T is the query)
        H_ctx = H[:, :-1, :]                          # (B, T-1, hidden)
        H_ctx = H_ctx.permute(0, 2, 1).contiguous()   # (B, hidden, T-1)

        # 2. CNN: extract temporal patterns
        C = F.relu(self.conv(H_ctx))                   # (B, n_filters, T-1)

        # 3. Global average pool over the time axis → (B, n_filters)
        C_pool = C.mean(dim=-1)                        # (B, n_filters)

        # 4. Score each filter against h_T
        #    W_score(h_T): (B, n_filters); multiply element-wise with C_pool
        score        = torch.sigmoid(self.W_score(h_T) * C_pool)   # (B, n_filters)
        attn_weights = torch.softmax(score, dim=-1)                 # (B, n_filters)

        # 5. Weighted sum of filter responses → context
        context_raw = (attn_weights.unsqueeze(-1) * C_pool.unsqueeze(-1)).sum(dim=1)
        # context_raw: (B, 1) — too small; project n_filters → hidden_size instead
...
class TPALSTMForecaster(nn.Module):
    ...
    def __init__(
        ...
        # Head receives h_T ‖ context  →  hidden_size * 2
    ...
    def forward(
        ...
        ----------
        x           : (B, T_in, F)
        return_attn : if True, also return attention weights (B, n_filters)
                      — used only during evaluation for visualisation

        Returns
        -------
        out  : (B, T_out)
        attn : (B, n_filters)  — only when return_attn=True
        """
        # LSTM encoder
        H, (h_n, _) = self.lstm(x)    # H: (B, T, hidden), h_n: (layers, B, hidden)
        h_T = h_n[-1]                  # top layer's final state: (B, hidden)

        # TPA attention
        context, attn = self.tpa(H, h_T)   # context: (B, hidden), attn: (B, n_filters)

        # Concatenate and project
        h_cat = torch.cat([h_T, context], dim=-1)   # (B, hidden*2)
        out   = self.head(h_cat)                    # (B, T_out)
...
def train_one_epoch(model, loader, optimiser, criterion, device) -> float:
    ...
    for X_b, y_b in loader:
        optimiser.zero_grad()
        loss = criterion(model(X_b), y_b)   # forward without attn during training
        loss.backward()
...
def collect_predictions_and_attention(
    ...
    """
    Run test set through model with return_attn=True.
    Returns (y_pred, y_true, attn_weights) all as numpy arrays.
    """
    model.eval()
    ...
    return (
        np.concatenate(preds_s),    # (N, T_out)
        np.concatenate(trues_s),    # (N, T_out)
        np.concatenate(attns),      # (N, n_filters)
    )
...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.plot(train_losses, label="Train MSE", linewidth=1.5, color="#0891b2")
    ax.plot(val_losses,   label="Val MSE",   linewidth=1.5, color="#f97316",
            linestyle="--")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("MSE loss (scaled)")
    ax.set_title("TPA-LSTM — Training curves")
...
def plot_predictions(
    ...
    """
    Two-panel overlay — identical layout to lstm_baseline.py and bilstm.py.
    ...
    C_ACTUAL = "#1d4ed8"
    C_PRED   = "#0891b2"   # teal — distinct from LSTM red and BiLSTM purple
    C_BAND   = "#a5f3fc"
    C_MID    = "#ea580c"
    C_LAST   = "#7c3aed"
    ...
    ax1.plot(idx_full, predicted_s1, color=C_PRED, linewidth=1.2,
             ...
             label="TPA-LSTM predicted (step 1)", zorder=5)
    ...
    ax1.set_title("TPA-LSTM — Test set: Actual vs Predicted (full period)",
...
def plot_per_step_metrics(per_step: list, out_path: str) -> None:
    ...
    ax1.set_title("TPA-LSTM — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²")
    ax2.set_title("TPA-LSTM — R² per horizon step", fontweight="bold")
    ax2.grid(alpha=0.3)
...
def plot_attention_heatmap(
    attn_weights: np.ndarray,    # (N_test, n_filters)
    out_path:     str,
    n_samples:    int = 200,
) -> None:
    """
    TPA-specific plot: heatmap of attention weights over test samples.

    Rows    = CNN filters (each one detects a distinct temporal pattern)
    Columns = test-set samples (time ordered)

    High values (bright) = this filter was highly relevant for that window.
    Persistent bright rows = patterns the model consistently relies on.
    Sparse activation = model uses few filters at a time (selective).
    """
    ...
    ax.set_title(
        "TPA Attention Weights — which filters activate per test window",
        fontweight="bold",
    )
    ax.set_yticks(np.arange(W.shape[0]))
    ax.set_yticklabels([f"Filter {i}" for i in range(W.shape[0])], fontsize=7)

    # ── Mean attention per filter (bar chart) ─────────────────────────────────
    ax2 = axes[1]
    mean_attn = attn_weights.mean(axis=0)     # (n_filters,)
    ...
    ax2.set_xlabel("CNN filter index")
    ax2.set_ylabel("Mean attention weight ± std")
    ax2.set_title(
        "Average filter importance across all test windows",
        fontweight="bold",
    )
    ax2.set_xticks(x)
    ax2.set_xticklabels([f"F{i}" for i in range(len(mean_attn))], fontsize=8)
...
def plot_three_way_comparison(
    ...
    """
    Five-panel 3-way comparison: LSTM / BiLSTM / TPA-LSTM.

    [0,0] Overall percentage metrics — grouped bars (up to 3 models)
    ...
    ax00.legend(fontsize=8); ax00.grid(axis="y", alpha=0.3)

    # ── [0,1] Overall R² ──────────────────────────────────────────────────────
    ax01 = fig.add_subplot(gs[0, 1])
    names_r2  = [m[0] for m in models]
    vals_r2   = [m[1]["R2"] for m in models]
    colors_r2 = [m[3] for m in models]
    bars_r2   = ax01.bar(names_r2, vals_r2, color=colors_r2, alpha=0.82, width=0.4)
    for bar in bars_r2:
        h = bar.get_height()
        ax01.text(bar.get_x() + bar.get_width() / 2, h + 0.004,
                  f"{h:.4f}", ha="center", va="bottom", fontsize=9)
    ax01.set_ylim(0, min(1.12, max(vals_r2) * 1.15 + 0.05))
...
def print_three_way_table(
    lstm_m: dict | None,
    bi_m:   dict | None,
    tpa_m:  dict,
) -> None:
    """
    Print a side-by-side overall metric table for all available models.
    Δ columns show TPA-LSTM minus each baseline.
    The winner column names the best model for each metric.
    """
    METRICS_CFG = [
    ...
    W   = 10
    sep_len = 16 + W * (1 + has_lstm + has_bi) + 18 * (has_lstm + has_bi) + 12
    sep = "─" * sep_len

    print(f"\n{'='*sep_len}")
    print("3-Way Comparison — Overall Test Metrics")
    print(f"{'='*sep_len}")
    ...
    for key, label, higher_better in METRICS_CFG:
        tpa_v  = tpa_m.get(key, float("nan"))
        fmt    = ".0f" if key in ("MAE", "RMSE") else (".4f" if key == "R2" else ".2f")

        row = f"{label:<14}"
        candidates = {"TPA-LSTM": tpa_v}
        ...
        if has_lstm:
            d = tpa_v - lstm_m.get(key, 0)
            row += f"  {'+' if d>=0 else ''}{d:{W}{fmt}}"
        if has_bi:
            d = tpa_v - bi_m.get(key, 0)
            row += f"  {'+' if d>=0 else ''}{d:{W}{fmt}}"

        # Winner: best value among available models
...
def main():
    ...
    y_pred_s, y_true_s, attn_weights = collect_predictions_and_attention(
        model, test_loader
    )
    ...
    W = 10
    print(f"\n{'='*85}")
    print("TPA-LSTM — TEST SET METRICS")
    print(f"{'='*85}")
    header = (
        f"{'Step':>5}  "
    ...
    out_dir = f"src/outputs/tpa_lstm/{run_id}"
    ...
    plot_attention_heatmap(attn_weights,
                           out_path=f"{out_dir}/attention_heatmap.png")

    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*85}")
    print(f"TPA-LSTM run complete  →  {out_dir}/")
    print(f"{'='*85}")
    ...
    print_three_way_table(lstm_m, bi_m, overall)
    ...
    plot_three_way_comparison(
        ...
        tpa_ps  = per_step[:min_steps],
        out_path= f"{out_dir}/comparison_three_way.png",
    )
