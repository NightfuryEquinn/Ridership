...
# ══════════════════════════════════════════════════════════════════════════════
# Metrics  (identical to lstm.py / bilstm.py / tpalstm.py)
# ══════════════════════════════════════════════════════════════════════════════
...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.plot(train_losses, label="Train MSE", linewidth=1.5, color="#16a34a")
...
def plot_four_way_comparison(
    ...
    """
    Six-panel 4-way comparison: LSTM / BiLSTM / TPA-LSTM / CNN-LSTM.

    [0,0] Overall percentage metrics — grouped bars (up to 4 models)
    ...
    models.append(                        ("CNN-LSTM", cnn_m,   cnn_ps,   C_CNN))
...
def print_four_way_table(
    ...
    Δ columns show CNN-LSTM minus each baseline.
    ...
    W       = 10
    n_prior = has_lstm + has_bi + has_tpa
    sep_len = 16 + W * (1 + n_prior) + 18 * n_prior + 12
    sep     = "─" * sep_len

    print(f"\n{'='*sep_len}")
    print("4-Way Comparison — Overall Test Metrics")
    print(f"{'='*sep_len}")
    ...
    for key, label, higher_better in METRICS_CFG:
        cnn_v = cnn_m.get(key, float("nan"))
        fmt   = ".0f" if key in ("MAE", "RMSE") else (".4f" if key == "R2" else ".2f")

        row        = f"{label:<14}"
        candidates = {"CNN-LSTM": cnn_v}
        ...
        if has_lstm:
            d = cnn_v - lstm_m.get(key, 0)
            row += f"  {'+' if d>=0 else ''}{d:{W}{fmt}}"
        if has_bi:
            d = cnn_v - bi_m.get(key, 0)
            row += f"  {'+' if d>=0 else ''}{d:{W}{fmt}}"
        if has_tpa:
            d = cnn_v - tpa_m.get(key, 0)
            row += f"  {'+' if d>=0 else ''}{d:{W}{fmt}}"
...
def main():
    ...
    print_four_way_table(lstm_m, bi_m, tpa_m, overall)
    ...
    plot_four_way_comparison(
        ...
        cnn_ps   = per_step[:min_steps],
        out_path = f"{out_dir}/comparison_four_way.png",
    )
