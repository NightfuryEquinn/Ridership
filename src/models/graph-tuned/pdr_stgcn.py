...
Architecture:
  X (B, T_in, N)
    → periodic diff encoder  → (B, N, 2, T_in)
...
def parse_args():
    p = argparse.ArgumentParser(description="Tuned PDR-STGCN forecaster with 12-way comparison")
...
class DynamicRelationalGraphConv(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, d_k: int = 64):
        super().__init__()
        self.d_k = d_k
...
class PDRSTGCNForecaster(nn.Module):
    """
    Periodicity-Aware Dynamic Relational STGCN forecaster (tuned).

    The symmetric-normalised adjacency A_sym is precomputed from training-data
    feature correlation and stored as a buffer.
...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.set_title("PDR-STGCN (Tuned) — Training curves")
...
def plot_predictions(y_true, y_pred, metrics, out_path: str) -> None:
    ...
    ax1.plot(idx_full, predicted_s1, color=C_PRED, linewidth=1.2, linestyle="--",
             alpha=0.88, label="PDR-STGCN (Tuned) predicted (step 1)", zorder=5)
    ann = (
    ...
    ax1.set_title("PDR-STGCN (Tuned) — Test set: Actual vs Predicted (full period)",
...
def plot_per_step_metrics(per_step: list, out_path: str) -> None:
    ...
    ax1.set_title("PDR-STGCN (Tuned) — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²"); ax2.set_title("PDR-STGCN (Tuned) — R² per horizon step", fontweight="bold")
    ax2.grid(alpha=0.3)
...
def main():
    ...
    print(f"\nModel           : PDRSTGCNForecaster (Tuned)")
    ...
    W = 10
    print(f"\n{'='*85}")
    print("PDR-STGCN (Tuned) — TEST SET METRICS")
    print(f"{'='*85}")
    ...
    out_dir = f"src/outputs/pdr_stgcn_tuned/{run_id}"
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*85}")
    print(f"PDR-STGCN (Tuned) run complete  →  {out_dir}/")
    print(f"{'='*85}")
    ...
    if len(models_data) > 1:
        print_comparison_table(models_data[:-1], overall, "PDR-STGCN (Tuned)")
    ...
    print(f"\n  Naive persistence  "
          ...
          f"R²={naive['R2']:.4f}")
    print(f"  PDR-STGCN (Tuned) vs naive  "
