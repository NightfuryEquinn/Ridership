...
class STFGNNBlock(nn.Module):
    ...
    def __init__(self, channels: int, dropout: float = 0.30):
        super().__init__()
...
class STFGNNForecaster(nn.Module):
    """
    Spatial-Temporal Fusion Graph Neural Network forecaster (tuned).
...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.set_title("STFGNN (Tuned) — Training curves")
...
def plot_predictions(y_true, y_pred, metrics, out_path: str) -> None:
    ...
    ax1.plot(idx_full, predicted_s1, color=C_PRED, linewidth=1.2, linestyle="--",
             alpha=0.88, label="STFGNN (Tuned) predicted (step 1)", zorder=5)
    ...
    ax1.set_title("STFGNN (Tuned) — Test set: Actual vs Predicted (full period)",
...
def plot_per_step_metrics(per_step: list, out_path: str) -> None:
    ...
    ax1.set_title("STFGNN (Tuned) — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²"); ax2.set_title("STFGNN (Tuned) — R² per horizon step", fontweight="bold")
    ax2.grid(alpha=0.3)
    fig.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"  Saved: {out_path}")
...
def main():
    ...
    W = 10
    print(f"\n{'='*85}")
    print("STFGNN (Tuned) — TEST SET METRICS")
    print(f"{'='*85}")
    ...
    out_dir = f"src/outputs/stfgnn_tuned/{run_id}"
    ...
    print(f"\n{'='*85}")
    print(f"STFGNN (Tuned) run complete  →  {out_dir}/")
    print(f"{'='*85}")
    ...
    if len(models_data) > 1:
        print_comparison_table(models_data[:-1], overall, "STFGNN (Tuned)")
