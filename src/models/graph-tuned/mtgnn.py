...
def parse_args():
    p = argparse.ArgumentParser(description="Tuned MTGNN forecaster with 9-way comparison")
...
class MTGNNForecaster(nn.Module):
    """
    Multi-Scale Temporal Graph Neural Network forecaster (tuned).
    ...
    def __init__(self, N: int, hidden: int, skip_ch: int, n_layers: int,
                 d_emb: int, d_hop: int, T_out: int, dropout: float = 0.15):
        super().__init__()
...
def plot_loss_curves(train_losses, val_losses, out_path: str) -> None:
    ...
    ax.set_title("MTGNN (Tuned) — Training curves")
...
def plot_predictions(y_true, y_pred, metrics, out_path: str) -> None:
    ...
    ax1.plot(idx_full, predicted_s1, color=C_PRED, linewidth=1.2, linestyle="--",
             alpha=0.88, label="MTGNN (Tuned) predicted (step 1)", zorder=5)
    ...
    ax1.set_title("MTGNN (Tuned) — Test set: Actual vs Predicted (full period)",
...
def plot_per_step_metrics(per_step: list, out_path: str) -> None:
    ...
    ax1.set_title("MTGNN (Tuned) — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²"); ax2.set_title("MTGNN (Tuned) — R² per horizon step", fontweight="bold")
    ax2.grid(alpha=0.3)
    fig.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"  Saved: {out_path}")
...
def main():
    ...
    W = 10
    print(f"\n{'='*85}")
    print("MTGNN (Tuned) — TEST SET METRICS")
    print(f"{'='*85}")
    ...
    out_dir = f"src/outputs/mtgnn_tuned/{run_id}"
    ...
    print(f"\n{'='*85}")
    print(f"MTGNN (Tuned) run complete  →  {out_dir}/")
    print(f"{'='*85}")
    ...
    if len(models_data) > 1:
        print_comparison_table(models_data[:-1], overall, "MTGNN (Tuned)")
