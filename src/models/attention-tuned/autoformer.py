...
class MovingAvg(nn.Module):
    """Learnable moving average for trend extraction."""
...
class AutoCorrelation(nn.Module):
    ...
    FFT-based time-delay correlation with explicit float32 cast for AMP
    stability.
    """
    ...
    def _time_delay_agg(self, V: torch.Tensor, corr: torch.Tensor) -> torch.Tensor:
        ...
        top_k = min(top_k, L)
        mean_corr = corr.abs().mean(dim=(1, 3))
        weights, delays = torch.topk(mean_corr, top_k, dim=-1)
        weights = torch.softmax(weights, dim=-1)
        out = torch.zeros_like(V)
        for k_idx in range(top_k):
            delay_k = delays[:, k_idx]
            w_k     = weights[:, k_idx].reshape(B, 1, 1, 1)
            rolled = torch.stack([
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor) -> torch.Tensor:
        ...
        nfft  = max(L_q, L_kv)
        Q_fft = torch.fft.rfft(Q_f, n=nfft, dim=2, norm="ortho")
        K_fft = torch.fft.rfft(K_f, n=nfft, dim=2, norm="ortho")
        corr_fft = Q_fft * torch.conj(K_fft)
        corr = torch.fft.irfft(corr_fft, n=nfft, dim=2, norm="ortho")
        corr = corr[:, :, :L_q, :]
        if L_kv != L_q:
        ...
        out = self._time_delay_agg(V_f, corr)
        out = out.to(Q.dtype)
...
class AutoformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, enc_out: torch.Tensor):
        ...
        trend_delta = self.trend_proj(trend1 + trend2 + trend3)
...
class AutoformerForecaster(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("enc_pe", self._sinusoidal_pe(T_in, d_model))
        T_dec = 2 * T_out
        self.register_buffer("dec_pe", self._sinusoidal_pe(T_dec, d_model))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        dec_seas  = torch.cat([x_seasonal[:, -T_out:, :], zeros], dim=1)
        dec_trend = torch.cat([x_trend[:, -T_out:, :],    zeros], dim=1)
        enc = self.enc_proj(x) + self.enc_pe[:, :T_in, :]
        for layer in self.enc_layers:
            enc = layer(enc)
        enc = self.enc_norm(enc)
        dec  = self.dec_proj(dec_seas) + self.dec_pe[:, :2*T_out, :]
        trend_accum = dec_trend
        for layer in self.dec_layers:
            dec, trend_delta = layer(dec, enc)
            trend_accum = trend_accum + trend_delta
        dec = self.dec_norm(dec)
        dec_out   = dec[:, -T_out:, :]
        trend_out = trend_accum[:, -T_out:, :]
...
def plot_predictions(y_true, y_pred, metrics, out_path):
    ...
    ax1.plot(idx_full, predicted_s1, color=C_PRED, linewidth=1.2, linestyle="--", alpha=0.88,
             label="Autoformer (tuned) predicted (step 1)", zorder=5)
    ...
    ax1.set_title("Autoformer (tuned) — Test set: Actual vs Predicted (full period)", fontsize=11, fontweight="bold")
...
def plot_per_step_metrics(per_step, out_path):
    ...
    ax1.set_title("Autoformer (tuned) — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²"); ax2.set_title("Autoformer (tuned) — R² per horizon", fontweight="bold")
    ax2.grid(alpha=0.3)
    fig.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(fig); print(f"  Saved: {out_path}")
...
def main():
    ...
    print(f"\nModel       : AutoformerForecaster (tuned)")
    ...
    W = 10; print(f"\n{'='*85}"); print("Autoformer (tuned) — TEST SET METRICS"); print(f"{'='*85}")
    ...
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S"); out_dir = f"src/outputs/autoformer_tuned/{run_id}"
    ...
    print(f"\n{'='*85}"); print(f"Autoformer (tuned) run complete  →  {out_dir}/"); print(f"{'='*85}")
    ...
    if len(models_data) > 1:
        print_comparison_table(models_data[:-1], overall, "Autoformer (tuned)")
    ...
    print(f"  Autoformer (tuned) vs naive ΔCombined={d_comb:+.2f}%  ΔMAPE={d_mape:+.2f}%  ΔR²={d_r2:+.4f}")
