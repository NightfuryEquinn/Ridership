...
class VariableSelectionNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, T, F = x.shape
        feat_flat  = self.feat_proj(x)
        feat_stack = feat_flat.reshape(B, T, F, self.d_model)
        sel_in      = self.select_grn(feat_flat)
        sel_weights = torch.softmax(self.select_fc(sel_in), dim=-1)
        out = (sel_weights.unsqueeze(-1) * feat_stack).sum(dim=2)
...
class TFTForecaster(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        vsn_in = self.vsn(x)
        h      = self.vsn_gan(vsn_in, vsn_in)

        lstm_in  = h
        lstm_out, _ = self.lstm(h)
        h        = self.lstm_gan(lstm_out, lstm_in)
        ...
        for attn, attn_gan, ffn, ffn_gan in zip(
            ...
            h           = attn_gan(attn_out, attn_in)
            ffn_in = h
            h      = ffn_gan(ffn(h), ffn_in)
...
def plot_loss_curves(train_losses, val_losses, out_path):
    ...
    ax.set_title("TFT (tuned) — Training curves")
...
def plot_predictions(y_true, y_pred, metrics, out_path):
    ...
    ax1.plot(idx_full, predicted_s1, color=C_PRED, linewidth=1.2, linestyle="--", alpha=0.88,
             label="TFT (tuned) predicted (step 1)", zorder=5)
    ...
    ax1.set_title("TFT (tuned) — Test set: Actual vs Predicted (full period)", fontsize=11, fontweight="bold")
...
def plot_per_step_metrics(per_step, out_path):
    ...
    ax1.set_title("TFT (tuned) — Per-horizon metrics", fontweight="bold")
    ...
    ax2.set_ylabel("R²"); ax2.set_title("TFT (tuned) — R² per horizon", fontweight="bold")
    ax2.grid(alpha=0.3)
    fig.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(fig); print(f"  Saved: {out_path}")
...
def main():
    ...
    W = 10; print(f"\n{'='*85}"); print("TFT (tuned) — TEST SET METRICS"); print(f"{'='*85}")
    ...
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S"); out_dir = f"src/outputs/tft_tuned/{run_id}"
    ...
    print(f"\n{'='*85}"); print(f"TFT (tuned) run complete  →  {out_dir}/"); print(f"{'='*85}")
    ...
    if len(models_data) > 1:
        print_comparison_table(models_data[:-1], overall, "TFT (tuned)")
    ...
    print(f"  TFT (tuned) vs naive        ΔCombined={d_comb:+.2f}%  ΔMAPE={d_mape:+.2f}%  ΔR²={d_r2:+.4f}")
