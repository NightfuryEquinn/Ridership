...
> **Methodological caveats (added 2026-06-11; see `REVISION.md` for the full audit).**
...
> 2. A feature-column-order defect was identified and fixed on 2026-06-11: SHAP feature labels in §4.8–4.9 were originally mislabelled (the label order did not match the tensor column order), HMT-TSF's semantic feature grouping was misaligned, and the known-future temporal tensor (`X_future`) carried lag/trend/fuel columns instead of calendar features. §4.8–4.9 below have been **relabelled with the correct feature names** (the underlying per-index SHAP values and consistency statistics are unaffected by the relabelling). HMT-TSF results in §4.7–4.9 pre-date the pipeline fix and are scheduled for regeneration; all baseline-model results (§4.2–4.6) are unaffected because baselines consume the feature vector order-agnostically and do not use `X_future`.
> 3. Pre-fix HMT-TSF runs gated the optional residual-boost decision on test-set metrics; the fixed implementation gates on validation metrics. The regenerated runs will reflect the corrected protocol.
...
> **Label correction (2026-06-11).** The SHAP labels originally reported in this section were generated by concatenating the semantic feature groups in metadata order, which does not match the tensor column order produced by `feature_align.py`. The per-index SHAP values and consistency statistics below are unchanged; only the feature names have been corrected to the true column at each index (e.g., feat_12 is `total_ridership`, not `rail_komuter`). The corrected mapping for every previously cited feature is tabulated in `REVISION.md`. SHAP plots (`shap_importance.png`) in existing run directories still carry the old labels and will be regenerated.

### 4.8.1 Top Feature Drivers

The ten highest-importance features reveal that the model's learned drivers are closely aligned with established transit demand theory. Historical ridership dominates the importance ranking: `total_ridership` (feat_12) — the system-wide aggregate that is itself the forecast target — is ranked first and appears in the top-15 features of all ten evaluated configurations (100% consistency), confirming strong target autocorrelation as the dominant predictive signal. `rail_lrt_kj` (LRT Kelana Jaya, feat_4) ranks second and `rail_mrt_kajang` (MRT Kajang, feat_3) third, reflecting the high-frequency daily co-movement patterns of the major Klang Valley rail corridors.

Calendar features emerge as the second-most important group: `is_weekend` (feat_42) ranks fourth and appears in the top-15 in 80% of configurations, and `days_since_last_public_hol` (feat_37) and `day_of_week` (feat_40) also appear in the top 10. These rankings confirm that the weekly commuting cycle and post-holiday return dynamics are first-order demand drivers — consistent with the EDA finding that weekday/weekend separation and holiday lead–lag effects produce the largest systematic ridership swings.

`fp_lv_diesel` (diesel price level, feat_20) is the highest-ranking fuel feature in the top 10 (rank 7, consistent in 60% of configurations), indicating a measurable but secondary mode-substitution signal. Penang rainfall (`rainfall_mm__MY07`, feat_53) ranks eighth, indicating that wet-weather conditions in a major urban transit market measurably shift mode choice toward rail and bus by reducing walkability and motorcycle viability.

### 4.8.2 Cross-Configuration Feature Stability

Aggregating SHAP importance across all ten configurations reveals a stable set of universally important features and a set of zero-importance features that are consistent regardless of look-back window or MCO condition. `total_ridership` is the only feature that appears in the top-15 of all ten configurations, establishing aggregate-demand autocorrelation as the single most reliable predictor across all operational regimes and window lengths. `is_weekend` (80% consistency) and `days_to_next_public_hol` (70%) are the most consistent calendar drivers, while the annual-cycle encodings `month_cos` (70%) and `month_sin` (60%) show that seasonal position carries stable predictive signal — plausibly tracking the school-term and festive-season demand cycle, including the northeast-monsoon months (November–January) identified in the rainfall EDA.

A lookback-dependent feature importance shift is also observed: at lb7, the very short context forces the model to rely more heavily on calendar position and external covariates, as there is insufficient temporal depth for autocorrelation to dominate. At lb14–28 (the optimal performance regime), a balanced feature profile emerges with ridership lines, calendar features, and external covariates contributing concurrently. At lb56–84, `total_ridership` dominates overwhelmingly, as the long autocorrelation window provides sufficient historical ridership depth for the model to anchor forecasts primarily on the recent ridership trajectory.

---

## 4.9 Feature-Reduced Variant: HMT-TSF-FR

Cross-run SHAP aggregation identified 23 features that contribute zero importance across all ten configurations (universal zeros) and three additional near-zero features (zero in 8 of 10 runs), totalling 26 removable features. Under the corrected label mapping (see §4.8 note), the 26 removed features are the entire Static group (all 17 features: population, GTFS, OSM POI, and GADM descriptors) plus nine fuel-price columns (`fp_lv_ron95`, `fp_lv_diesel_eastmsia`, `fp_lv_ron95_budi95`, `fp_lv_ron95_skps`, `fp_lv_ron95_pct_chg`, `fp_chg_ron95`, `fp_chg_diesel_eastmsia`, `fp_chg_ron95_budi95`, `fp_chg_ron95_skps`). The static group contributes no discriminative signal because time-invariant structural capacity is already implicitly encoded in historical ridership levels: high-capacity corridors persistently exhibit high ridership throughout the study period, making static spatial descriptors redundant given sufficient look-back context. The nine inactive fuel columns are administered prices that were frozen or near-constant across the post-MCO training window (RON95 was held at RM2.05 throughout), or East Malaysia variants irrelevant to the Peninsular ridership base — near-zero day-to-day variance leaves them nothing to contribute.
...
**Fifth, SHAP analysis confirms that the model's learned feature importance hierarchy is interpretable and domain-consistent.** Historical ridership autocorrelation, calendar structure (weekend flag, day-of-week, holiday lead–lag, seasonal position), and secondary fuel-price and rainfall signals are the primary predictive drivers, while the entire static feature group and the near-constant administered fuel prices are confirmed as redundant given sufficient temporal context. The 53-feature HMT-TSF-FR variant matches the standard model's headline performance within 0.05 Combined% and is recommended for `nomco` operational deployments.
